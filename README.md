...
- 台账预览可在本地勾选或取消资产，选择会在后续只读同步中保留；远端曾移除后重新出现的资产会重新纳入盘点。
- 资产统一显示在一个可筛选清单中；批量模式支持全选当前结果，并将一次拍摄的照片关联到多项资产、分别生成盘点 PDF。
...
最新安装包：[GitHub Releases](https://github.com/tentyou/JPT/releases/latest)。当前自动发布的是调试签名 APK；正式分发签名仍按上文的签名配置单独构建。
