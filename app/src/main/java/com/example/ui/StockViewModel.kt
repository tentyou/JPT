...
class StockViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun deleteSharedPhotoForAll(file: File) {
        val activeItem = _activeItemForPhoto.value ?: return
        viewModelScope.launch(Dispatchers.IO) {
            val affected = com.example.data.SharedPhotoLinks.deleteForAll(file, context.filesDir)
            affected.forEach { uid ->
                val item = repository.getItemByUid(uid) ?: return@forEach
                val count = repository.countPhotos(context, uid)
                if (count > 0) repository.generatePdfForItem(context, item)
                else {
                    File(context.filesDir, "pdfs/$uid").deleteRecursively()
                    repository.updatePhotoState(uid, 0, "未生成")
                }
            }
            withContext(Dispatchers.Main) {
                refreshActiveSessionPhotos(activeItem.uid)
                Toast.makeText(context, "共享照片已从 ${affected.size} 项资产删除。", Toast.LENGTH_SHORT).show()
            }
        }
    }
